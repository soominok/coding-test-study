...
- DFS는 __깊이 우선 탐색__ 이라고도 부르며 그래프에서 깊은 부분을 우선적으로 탐색하는 알고리즘
- DFS는 __스택 자료구조(혹은 재귀 함수)를 이용__ 하며, 구체적인 동작 과정은 다음과 같음
   
      1. 탐색 시작 노드를 삽입하고 방문 처리를 함
...
### ■ DFS 소스코드 예제
      ...
      # 모든 값들이 False로 초기화하여 모든 노드를 아직 하나도 방문하지 않은 것처럼 처리
      # 인덱스 0은 사용하지 않기 위해서 하나 더 큰 값인 9
      visited = [False] * 9
...
### ■ BFS (Breadth-First Search) ★★★ (대기업 코딩테스트에 많이 출제)
- __너비 우선 탐색__ 이라고도 부르며, 그래프에서 __가까운 노드부터 우선적으로 탐색하는 알고리즘__
- __큐 자료구조__ 를 이용, 구체적인 동작 과정은 다음과 같음

      1. 탐색 시작 노드를 큐에 삽입하고 방문 처리
      2. 큐 에서 노드를 꺼낸 뒤에 해당 노드의 인접 노드 중에서 방문하지 않은 노드를 모두 큐에 삽입하고 방문 처리함
      3. 더 이상 2번의 과정을 수행할 수 없을 때까지 반복
      
__[DFS를 수행할 때]__
- 인접하지 않은 노드에 대해서 다시 한번씩 큐에 넣으면서 수행

__[BFS를 수행할 때]__
- 해당 시점에서 인접한 노드를 한번에 전부 큐에 넣는 것이 특징
- 특정 조건에서의 최단 경로 문제를 해결하기 위한 목적으로도 효과적으로 사용


### ■ BFS 소스코드 예제

      form collections from deque
