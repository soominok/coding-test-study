...
- 연속적인 위치를 갖는 원소들을 가져와야 할 때는 슬라이싱(Slicing)을 이용
  ...
  * __끝 인덱스는 실제 인덱스보다 1을 더 크게 설정 ★★★__
...
sort()|변수명.sort()|기본정렬 기능으로 오름차순으로 정렬한다.|O(NlogN) 
...
- 파이썬의 사전 자료형은 해시 테이블(Hash Table)을 이용하므로 __데이터의 조회 및 수정에 있어서 O(1)의 시간에 처리__ 할 수 있음
  ...
  ※ 특정한 키(Key)가 존재하는지 검사하기 위해서 상수시간이 소요되기 때문에 
  
  ※ 문자열과 같은 키를 이용해서 데이터를 저장하고 관리하는데 있어서 리스트보다 데이터 조회를 더 효율적으로 할 수 있음
...
- 사전 자료형과 집합 자료형은 순서가 없기 때문에 인덱싱으로 값 없을 수 X
  ...
  ※ 사전의 키(Key) 혹은 집합의 원소(Element)를 이용해 O(1)의 시간 복잡도로 조회 가능
      ...
      ※ 이때, 키(Key) 혹은 원소(Element)의 값으로는 문자열(str)이나 튜플(tuple)과 같은 객체가 사용되어야 함
